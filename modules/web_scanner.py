...
# Désactive les avertissements pour les requêtes non vérifiées (pour verify=False)
from requests.packages.urllib3.exceptions import InsecureRequestWarning
requests.packages.urllib3.disable_warnings(InsecureRequestWarning)

def scan_headers(url):
    """
    Récupère et analyse les en-têtes HTTP d'une URL,
    en identifiant des indices de sécurité et des technologies.
    """
    print(colored(f"[WEB SCAN] Récupération et analyse des en-têtes HTTP pour {url}...", 'cyan'))
    try:
        # Utilise requests.get pour avoir accès au contenu si nécessaire pour des en-têtes dynamiques
        # et suit les redirections. Un User-Agent est essentiel.
        ...
        return response.headers, response.text # Retourne aussi le contenu pour la détection technologique
...
def advanced_tech_detection(headers, content=""):
    """
    Détection de technologies web plus avancée basée sur les en-têtes et le contenu HTML.
    """
    ...
    via = headers.get('Via', '').lower() # Souvent utilisé par les proxys/CDNs
    ...
    # Détection basée sur le contenu HTML (si disponible)
    if content:
...
def check_sitemap_xml(url):
    """Vérifie et affiche le contenu de sitemap.xml."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Affiche seulement les 500 premiers caractères pour ne pas surcharger la console
...
def check_sensitive_files(url):
    """
    Vérifie la présence de fichiers sensibles courants en utilisant des requêtes HEAD.
    """
    sensitive_paths = [
...
def scan_known_vulnerabilities(url):
    """Recherche de chemins connus associés à des vulnérabilités ou des configurations faibles."""
    vulnerable_paths_and_signatures = {
        # WordPress
        '/wp-admin/install.php': 'Congratulations, you have successfully installed WordPress!',
        '/wp-config.php.bak': 'DB_NAME', # Potentiel backup de config
        '/wp-content/debug.log': 'WordPress debug log', # Peut contenir des erreurs ou infos sensibles

        # Joomla
        '/administrator/install.php': 'Joomla! Installation',

        # Général
        '/user/register': 'Create new account', # Indice d'inscription ouverte
        '/vendor/phpunit/phpunit/src/Util/PHP/eval-stdin.php': 'PHPUnit', # PHPUnit RCE (CVE-2017-9841)
        '/web.config': '<configuration>', # Potentiel disclosure de configuration (IIS)
        '/.git/HEAD': 'ref: refs/heads/', # Index d'un répertoire .git exposé
        '/server-status': 'Apache Server Status', # Status serveur Apache exposé
        '/phpmyadmin/': 'phpMyAdmin', # Chemin courant pour phpMyAdmin
        '/admin/pma/': 'phpMyAdmin',
        '/_admin/': '', # Répertoire admin générique
        '/backup/': '', # Répertoire de backup générique
        '/test/': '', # Répertoire de test générique
    }
    ...
    for path, signature in vulnerable_paths_and_signatures.items():
        check_url = urljoin(url, path)
        try:
            # Utilise GET car nous devons analyser le contenu
            ...
            if response.status_code == 200:
                ...
                if signature and signature in response.text:
                    ...
                    found_any_vuln = True
                elif not signature: # Cas où il n'y a pas de signature spécifique, juste la présence du chemin
            ...
            # Ne pas afficher une erreur si la connexion est simplement refusée ou introuvable (404)
...
def scan_web_hardcore(target_url):
    ...
    if not parsed_target.scheme:
        ...
        if not headers: # Si HTTPS échoue, tente HTTP
            ...
            target_url = target_url_http # Met à jour l'URL si HTTP a réussi
        else:
            target_url = target_url_https # Garde l'URL HTTPS si elle a réussi
    ...
    if headers:
        # Poursuite du scan si les en-têtes ont été récupérés (signe de connexion réussie)
        advanced_tech_detection(headers, content)
        check_robots_txt(target_url)
        check_sitemap_xml(target_url) # Ajout de la vérification du sitemap
...
def scan_web_hardcore_main():
    """Fonction principale pour l'exécution du module Web Scanner."""
...
if __name__ == "__main__":
    # Ceci s'exécute si webscanner.py est lancé directement, pas via HASHISH.py
