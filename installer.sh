...
# Assurons-nous que hashish.py dans INSTALL_DIR a les bonnes permissions
if [ -f "$INSTALL_DIR/hashish.py" ]; then
    ...
    echo -e "${YELLOW}Avertissement : $INSTALL_DIR/hashish.py non trouvé pour les permissions finales.${NC}"
fi

# Assurons-nous que l'exécutable C++ a les bonnes permissions
if [ -f "$MODULES_TARGET_DIR/hashcracker" ]; then
    ...
    echo -e "${YELLOW}Avertissement : $MODULES_TARGET_DIR/hashcracker non trouvé pour les permissions finales.${NC}"
fi
...
echo -e "${GREEN}Permissions d'exécution accordées aux fichiers spécifiés.${NC}\n"
