...
# clang sur Termux fournit le compilateur g++ et les outils de construction nécessaires
if ! command -v g++ &> /dev/null; then
    echo -e "${YELLOW}Compilateur g++ non trouvé. Installation de 'clang' (qui inclut g++ sur Termux)...${NC}"
    install_package "clang" || { echo -e "${RED}Installation annulée. 'clang' est nécessaire pour compiler les modules C++.${NC}"; exit 1; }
fi

if ! pkg list-installed | grep -q "^openssl/"; then
    echo -e "${YELLOW}OpenSSL non trouvé. Nécessaire pour la compilation C++. Installation de 'openssl' et 'libcrypt-dev', 'libssl-dev'...${NC}"
    install_package "openssl" || { echo -e "${RED}Installation annulée. 'openssl' est nécessaire.${NC}"; exit 1; }
    install_package "libcrypt-dev" || { echo -e "${RED}Installation annulée. 'libcrypt-dev' est nécessaire.${NC}"; exit 1; }
    install_package "libssl-dev" || { echo -e "${RED}Installation annulée. 'libssl-dev' est nécessaire.${NC}"; exit 1; }
else
    echo -e "${GREEN}OpenSSL et les bibliothèques de développement SSL sont déjà installés.${NC}"
fi
...
if ! command -v g++ &> /dev/null; then
  ...
  echo -e "${YELLOW}Veuillez résoudre manuellement le problème d'installation de 'clang' dans Termux et relancer le script.${NC}"
  exit 1
fi
