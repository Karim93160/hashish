...
if command -v rsync &> /dev/null; then
    ...
    # Exclure les fichiers C++ lors de la copie des modules Python pour les gérer séparément
    rsync -av --exclude 'wordlists/' --include='*.py' --include='*/' --exclude='*.cpp' --exclude='*.h' --exclude='*' "$REPO_PATH/modules/" "$MODULES_TARGET_DIR/" || { echo -e "${YELLOW}Avertissement : Erreur lors de la copie des modules Python avec rsync. Vérifiez le dossier '$REPO_PATH/modules/'.${NC}"; }
else
    ...
    for dir in "$REPO_PATH/modules"/*/; do
        ...
        if [ "$dir_name" != "wordlists" ]; then
            # Copier récursivement mais exclure les fichiers C++ si possible (find -name ne supporte pas --exclude)
...
HASH_RECON_H_SOURCE="$REPO_PATH/modules/hash_recon.h" # Ajout du .h
...
if [ -f "$HASHCRACKER_CPP_SOURCE" ] && [ -f "$HASH_RECON_CPP_SOURCE" ] && [ -f "$HASH_RECON_H_SOURCE" ]; then
  ...
  echo -e "${INFO}Copie de hash_recon.h et hash_recon.cpp vers $MODULES_TARGET_DIR pour la compilation...${NC}"
  ...
  echo -e "${CYAN}Commande de compilation : g++ \"$MODULES_TARGET_DIR/hashcracker.cpp\" \"$MODULES_TARGET_DIR/hash_recon.cpp\" -o \"$HASHCRACKER_FINAL_EXECUTABLE\" -lcrypto -lssl -std=c++17 -fopenmp -pthread -I\"$MODULES_TARGET_DIR\" -I\"$OPENSSL_INCLUDE_PATH\" -L\"$OPENSSL_LIB_PATH\" ${NC}"

  # La commande de compilation cible les fichiers dans MODULES_TARGET_DIR et inclut ce répertoire
  if g++ "$MODULES_TARGET_DIR/hashcracker.cpp" "$MODULES_TARGET_DIR/hash_recon.cpp" -o "$HASHCRACKER_FINAL_EXECUTABLE" \
  ...
  echo -e "${YELLOW}Un ou plusieurs fichiers source C++ (hashcracker.cpp, hash_recon.cpp, hash_recon.h) n'ont pas été trouvés dans '$REPO_PATH/modules'. La compilation C++ est ignorée.${NC}"
  echo -e "${YELLOW}Le module Hash Cracker C++ ne sera PAS disponible ou ne fonctionnera pas correctement.${NC}"
fi
